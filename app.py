...
from llm_agent import llm_query, recommendation_query, analyst_query
from toolbox import *
from recommendation_metadata import extract_dataset_metadata
from analyst_metadata import extract_metadata
from sqldb_config import save_dataset_to_db

st.title("🧠 PAIDA - Your Personal AI Data Analyst")

st.sidebar.title("Choose your task")
selectbox = st.sidebar.selectbox("Select your task", ["Modify your Dataset", "Know about your Dataset", "Recommend Models for your Dataset"])

if selectbox == "Modify your Dataset":
    st.subheader("Modify your dataset using LLM-powered transformations.")
    ...
    if uploaded_file is not None:
        ...
        st.divider()
        st.subheader("💾 Save Modified Dataset to SQL Database")

        table_name = st.text_input("Enter the name of the table to save in SQL database:")

        if st.button("Save to SQL Database"):
            save_dataset_to_db(st.session_state["df"], table_name)
            st.success("Dataset saved to SQL database successfully!")

elif selectbox == "Know about your Dataset":
    st.subheader("Get insights about your dataset using LLM-powered analysis.")
    uploaded_file = st.file_uploader("Upload your Dataset in CSV, JSON or Excel format", type=["csv", "json", "xlsx"])
    df = pd.DataFrame()
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Analyzing dataset..."):
            analyst_insights = analyst_query(metadata)
            st.write("📈 Analyst Insights")
            st.write(analyst_insights)

elif selectbox == "Recommend Models for your Dataset":
    st.subheader("Get Ml / DL model recommendations for your dataset using LLM-powered analysis.")
    uploaded_file = st.file_uploader("Upload your Dataset in CSV, JSON or Excel format", type=["csv", "json", "xlsx"])
    df = pd.DataFrame()
